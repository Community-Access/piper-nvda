...
```
certutil -hashfile dist\piper-neural-0.5.0.nvda-addon SHA256
```

## Step 3: publish the file

Create a GitHub release (tag it, e.g. `v0.5.0`) and attach the
...
| `addonVersionName` | the version string, e.g. `0.5.0` |
...
Known gaps at 0.5.0, none of which block a GitHub release but all of which are
